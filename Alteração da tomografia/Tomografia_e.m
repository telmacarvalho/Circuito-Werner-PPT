...
x1 = pi/8+(randi([-1 1],1,1)*rand(1,1)*pi/180);
x2 = 0;
y1 = pi/8+(randi([-1 1],1,1)*rand(1,1)*pi/180);
y2 = 0;
y3 = -pi/2+(randi([-1 1],1,1)*rand(1,1)*pi/180);
...
rho = Entrada*(Entrada)';
    for j=1:16
        assert(Rho(j) < (rho(j)+exp(-3)) && Rho(j) > (rho(j)-exp(-3)))
    end
end